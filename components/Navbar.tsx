...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    function naTaster(e: KeyboardEvent) {
      if (e.key === "Escape") setMenuOtvoren(false);
    }
  ...
  return (
    <nav ref={menuRef} className="bg-white border-b border-gray-200 sticky top-0 z-50">
            ...
            {korisnik ? (
              <>
                ...
                <div className="relative">
                  ...
                  >
                    <div className="w-7 h-7 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold" aria-hidden="true">
                      {korisnik.ime[0].toUpperCase()}
                    </div>
                    <span className="font-medium">{korisnik.ime}</span>
                  </button>
                  {menuOtvoren && (
                    <div role="menu" className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1">
                      <Link
                        href="/profil"
                        role="menuitem"
